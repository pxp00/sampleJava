...
//reqs: annotation of constructor, field, mtd
//flow: isAnnotationPresent(xxx_Annotation.class), getAnnotation(xxx_Annotation.class), 
//      Annotation[][] array = getParameterAnnotations() //num_parameters, num_annotations 
public class Main_05 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < declaredConstructors.length; i++) {
			...
			for (int j = 0; j < parameterAnnotations.length; j++) { // num_parameters
				int length = parameterAnnotations[j].length; // num_annotations
				System.out.println(" parameterAnnotations index =  " + j + "\n num_annotations = " + parameterAnnotations[j].length);
				if (length == 0) // without annotation
